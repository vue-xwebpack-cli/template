...
export const combineDuration = (duration = [], format = DATE_FORMAT2, hyphen = '至') =>
...
export const dateTimeRepeatCheck = (arr, formatString, date, spliter) => {
  if (arr === undefined || arr.length < 0) {
    return true
  }
  const currStart = date.split(spliter)[0]
  const currEnd = date.split(' ')[0] + ' ' + date.split(spliter)[1]
  for (const time of arr.values()) {
    const start = time.split(spliter)[0]
    const end = time.split(' ')[0] + ' ' + time.split(spliter)[1]
    if (moment(currStart).isBefore(moment(start)) && moment(currEnd).isAfter(moment(start))) {
      return false
    }
    if (moment(currStart).isAfter(moment(start)) && moment(currStart).isBefore(moment(end))) {
      return false
    }
    if (currStart === start && moment(currEnd).isAfter(moment(start))) {
      return false
    }
    if (currStart === start && moment(currStart).isBefore(moment(end))) {
      return false
    }
  }
  return true
